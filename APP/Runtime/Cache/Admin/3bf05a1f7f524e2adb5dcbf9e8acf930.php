...
<body style="padding:20px 20px 0px 20px">
	...
	<table id="dg" title="讲座列表" style="width:1200px;height:380px" >
	<?php if(is_array($lecture)): foreach($lecture as $key=>$u): ?><tr>
		...
		<td>
			<a href="<?php echo U('Admin/Stu/mylectureread',array('lid'=>$u['lid'],'unum'=>$unum,'uname'=>$uname));?>">读取</a>
			<a href="<?php echo U('Admin/Stu/mylectureapply',array('lid'=>$u['lid'],'unum'=>$unum,'uname'=>$uname,));?>">查看报名</a>
			<?php if($u['lcheckstatus'] == 1): else: ?>
				<a href="javascript:void(0)" onclick="mylecturedelete(<?php echo ($u["lid"]); ?>)">删除</a><?php endif; ?>
		</td>		
