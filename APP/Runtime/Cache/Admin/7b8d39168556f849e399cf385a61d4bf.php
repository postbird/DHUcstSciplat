...
<body>
	...
	<hr/>
	 <form id="form2" method='post' action="__URL__/mylectureupdate" class="form-horizontal" enctype="multipart/form-data">
			...
			<div class="form-group">
	  		...
	  		<div class="form-group">
			    ...
			    <label for="username" class="col-sm-2 control-label" >审核状态:</label>
	  			<div class="col-sm-4">
			    	<?php if($lecture['lcheckstatus'] == 0): ?>未审核
			    	<?php else: ?>
			    		<?php if($lecture['lcheckstatus'] == 1): ?>已通过
			    		<?php else: ?>
			    			未通过<?php endif; endif; ?>
			    </div>
  			</div>
		 <div class="modal-footer">
	  		...
	  		<input type='hidden',id='unum' name='unum' value='<?php echo ($unum); ?>'>
   			<input type='hidden',id='uname' name='uname' value='<?php echo ($uname); ?>'>
		   	 <a class="btn btn-primary" href="<?php echo U('Admin/Stu/mydirectlecture',array('unum'=>$unum,'uname'=>$uname));?>" target="opt"><span>返回</span></a>
		   	 <?php if($lecture['lcheckstatus'] == 1): ?><input type="submit" class="btn btn-primary" value="保 存" >
		   	 <?php else: ?>
