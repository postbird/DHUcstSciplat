...
<body>
	<div class='container'>
	<h3 class='head'><?php echo ($race[0]['race_name']); ?></h3>
	<hr/>
		<form  method='post' action="__URL__/myraceupdate" class="form-horizontal" enctype="multipart/form-data">
			<div class="form-group">
				<label for="usernum" class="col-sm-2 control-label" >详细信息:</label>
			 	<div class="col-sm-11 col-sm-offset-1 ">
				 	...
				 	<div class="col-sm-11 col-sm-offset-1">
				 		<div class="col-sm-3 inputpadding">
							<select id="bonus" name="bonus" class="form-control">
				   		 		...
				   		 		<?php if($u['bonus'] == '一等奖'): ?><option value='一等奖'  selected="selected">一等奖</option>
				   		 		<?php else: ?>
				   		 			<option value='一等奖'>一等奖</option><?php endif; ?>	
				   		 		<?php if($u['bonus'] == '二等奖'): ?><option value='二等奖'  selected="selected">二等奖</option>
				   		 		<?php else: ?>
				   		 			<option value='二等奖'>二等奖</option><?php endif; ?>	
				   		 		<?php if($u['bonus'] == '三等奖'): ?><option value='三等奖'  selected="selected">三等奖</option>
				   		 		<?php else: ?>
				   		 			<option value='三等奖'>三等奖</option><?php endif; ?>	
				   		 		<?php if($u['bonus'] == '参与奖'): ?><option value='参与奖'  selected="selected">参与奖</option>
				   		 		<?php else: ?>
					...
					<label for="username" class="col-sm-1  control-label" >图片上传:</label>
						<div class="col-sm-11 col-sm-offset-1">
					     ...
					     <input type='file' name="imageFile" onchange="previewFile()" id='imageread'/>
			      		<img src="__ROOT__<?php echo ($u['image']); ?>" height="200" width="300" alt="图片 预览..."/>	
					    </div>
					...
					<div class="col-sm-11 col-sm-offset-1">
					     ...
					     当前文件为：<?php echo ($u["accessory"]); ?><br/>
						<div class="col-sm-8 inputpadding">
					        <?php if($u['accessory'] == ''): else: ?>
						    	 <a href="<?php echo U('Admin/SuperManage/downracefile');?>?filename=.<?php echo ($u['accessory']); ?>">附件下载</a><?php endif; ?>   			    	
					    </div>		
					</div>
					<label for="usernum" class="col-sm-1 control-label" >审核:</label>
					<div class="col-sm-11 col-sm-offset-1">
						<div class="col-sm-8 inputpadding">
							<?php if($u['status'] == 0): ?>未审核
						    <?php else: ?>
								<?php if($u['status'] == 1): ?>已通过
					       		<?php else: ?>
									未通过<?php endif; endif; ?>
						</div>	
					</div>	
		 	...
		 	<input type='hidden' name='racefile' value='<?php echo ($u["accessory"]); ?>'>
		 	<input type='hidden' name='imagefile' value='<?php echo ($u["image"]); ?>'>
   			<input type='hidden',id='race_id' name='race_id' value="<?php echo ($u['race_id']); ?>">
   			<input type='hidden',id='captainnum' name='captainnum' value="<?php echo ($u['captainnum']); ?>">
   			<input type='hidden',id='unum' name='unum' value='<?php echo ($unum); ?>'>
   			<input type='hidden',id='uname' name='uname' value='<?php echo ($uname); ?>'>
   			<a class="btn btn-primary" href="<?php echo U('Admin/Stu/myrace',array('unum'=>$unum,'uname'=>$uname));?>" target="opt"><span>返回</span></a>
	   	 	<?php if($u['status'] == 1): ?>&nbsp;
	   	 	<?php else: ?>
	   	 		<input type="submit" class="btn btn-primary" value="保 存" id="adduserbtn" name="adduserbtn"><?php endif; ?>
	   </div>
	   <hr/>
	</form>	   
