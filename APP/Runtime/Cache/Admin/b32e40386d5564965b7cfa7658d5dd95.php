...
<body style="padding:20px 20px 0px 20px">
	...
	<a class="btn btn-default "  href="__URL__/addLecture/unum/<?php echo ($unum); ?>/uname/<?php echo ($uname); ?>">
					发布讲座
			</a>
	...
	<table id="dg" title="讲座列表" style="width:1280px;height:380px" >
	<?php if(is_array($lecture)): foreach($lecture as $key=>$u): ?><tr>
		...
		<td><?php echo ($u["lplace"]); ?></td>
		<td><?php echo ($u["ldirectorname"]); ?></td>
		...
		<td><?php echo ($u["lnum"]); ?></td>
		<td>
			<?php if($u["lcheckstatus"] == 0): ?>未审核
			<?php else: ?>
				<?php if($u["lcheckstatus"] == 1): ?>已通过
				<?php else: ?>
					未通过<?php endif; endif; ?>
		</td>
		<td>
			<?php if($u["lstatus"] == 1): ?>是
			<?php else: ?>
				否<?php endif; ?>
		</td>
		<td>
			<a href="<?php echo U('Admin/SuperManage/lectureread',array('lid'=>$u['lid'],'unum'=>$unum,'uname'=>$uname));?>">读取</a>
			<a href="<?php echo U('Admin/SuperManage/lectureapply',array('lid'=>$u['lid'],'unum'=>$unum,'uname'=>$uname,));?>">查看报名</a>
			<a href="javascript:void(0)" onclick="deletelecture(<?php echo ($u["lid"]); ?>)">删除</a>
		</td>		
