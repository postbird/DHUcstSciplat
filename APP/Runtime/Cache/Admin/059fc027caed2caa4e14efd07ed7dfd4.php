...
</script><script type="text/javascript" src="__ROOT__/Data/Ueditor/ueditor.config.js"></script><script type="text/javascript" src="__ROOT__/Data/Ueditor/ueditor.all.min.js"></script></head><body style="padding:20px 20px 0px 20px"><h2 align="center">信息列表</h2><hr/><br/><!-- data-toggle="modal" data-target="#adduserModal" --><a class="btn btn-default "  href="__URL__/addNews">
					发布信息
			</a><div style="margin:10px 0;"></div><form method='post' action="__URL__/newslistset" ><table id="dg" title="信息列表" style="width:800px;height:380px" ><?php if(is_array($news)): foreach($news as $key=>$u): ?><tr><td><?php echo ($u["nid"]); ?></td><td><input type="checkbox" value="<?php echo ($u["nid"]); ?>" name="subBox[]" class="sub-box"></td><td><?php echo ($u["ntitle"]); ?></td><td><?php echo ($u["ndate"]); ?></td><td><?php echo ($u["npublishname"]); ?></td><td><?php if($u["nstatus"] == 1): ?>是
				<?php else: ?>
					否<?php endif; ?></td><td><?php if($u["ntop"] == 1): ?><span class='red'>是</span><?php else: ?>
					否<?php endif; ?></td><td><a href="<?php echo U('Admin/SuperManage/newsread',array('nid'=>$u['nid'],'unum'=>$unum,'uname'=>$uname));?>">读取</a><a href="javascript:void(0)" onclick="deletenews(<?php echo ($u["nid"]); ?>)">删除</a></td></tr><?php endforeach; endif; ?></table><div style="padding-left:300px;"><?php echo ($page); ?></div><a id="checkAll" href="javascript:;"class="btn btn-default">全选</a><a id="delAll" href="javascript:;" class="btn btn-default">取消全选</a><input type="submit" name='option1' value="选中置顶" class="btn btn-primary"><input type="submit" name='option2' value="取消置顶" class="btn btn-primary">
		&nbsp;&nbsp;<span class='red'><?php echo ($msg); ?></span></form><!-- Modal 添加信息--><div class="modal fade" id="adduserModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"  ><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title" id="myModalLabel">发布信息</h4></div><div class="modal-body"><form id="form1" method='post' action="__URL__/newsinsert" class="form-horizontal" enctype="multipart/form-data"><div class="form-group"><div class="col-sm-8 col-sm-offset-2"><input type='text' class="form-control " id="title" name="title"  placeholder="标题"></div><hr/><div class="col-sm-10 col-sm-offset-1"><textarea  id="content" name="content" ></textarea></div></div><div class="form-group"><label for="username" class="col-sm-2  control-label" >附件:</label><div class="col-sm-8">
			     请选择需要上传的文件(大小不超过10M，文件类型为.rar,.zip)：
			     <input type='file' name="newsFile"/></div></div><div class="modal-footer"><input type='hidden',id='unum' name='unum' value='<?php echo ($unum); ?>'><input type='hidden',id='uname' name='uname' value='<?php echo ($uname); ?>'><input type="submit" class="btn btn-primary" value="保 存" id="adduserbtn" name="adduserbtn"></div></form></div></div></div></div><script>	function deletenews(nid){
		alert("删除后不可恢复，确认删除？");
		if(confirm("确认删除")){
			$.ajax({
				type:"POST",
