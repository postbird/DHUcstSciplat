...
<body style="padding:20px 20px 0px 20px">
	  ...
	  <tr>
	  	...
	  	<td>
	  		<select id="searchannual" name="searchannual" class="form-control" >
	  			<?php if($searchyear == 0): ?><option value="0" selected="selected">=请选择年度=</option>
	  			<?php else: ?>
	  				<option value="0">=请选择年度=</option><?php endif; ?>
	  			<?php $__FOR_START_24294__=2014;$__FOR_END_24294__=2034;for($i=$__FOR_START_24294__;$i < $__FOR_END_24294__;$i+=1){ if($searchyear == $i): ?><option value="<?php echo ($i); ?>" selected="selected"><?php echo ($i); ?></option>
		  			<?php else: ?>
	  	...
	  	<td>
	  		<select id="searchlevel" name="searchlevel" class="form-control" >
	  			<?php if($searchlevel == '0'): ?><option value="0" selected="selected">=请选择级别=</option>
	  			<?php else: ?>
	  				<option value="0">=请选择级别=</option><?php endif; ?>
	  			<?php if($searchlevel == '国家级'): ?><option value="国家级" selected="selected">国家级</option>
	  			<?php else: ?>
	  				<option value="国家级">国家级</option><?php endif; ?>
	  			<?php if($searchlevel == '上海市级'): ?><option value="上海市级" selected="selected">上海市级</option>
	  			<?php else: ?>
	  				<option value="上海市级">上海市级</option><?php endif; ?>
	  			<?php if($searchlevel == '校级'): ?><option value="校级" selected="selected">校级</option>
	  			<?php else: ?>
	  				<option value="校级">校级</option><?php endif; ?>
	  			<?php if($searchlevel == '院级'): ?><option value="院级" selected="selected">院级</option>
	  			<?php else: ?>
	  	...
	  	<td>
	  		<select id="searchfid" name="searchfid" class="form-control" >
	  			<?php if($searchfid == 0): ?><option value="0" selected="selected">=请选择来源=</option>
	  			<?php else: ?>
	  				<option value="0">=请选择来源=</option><?php endif; ?>
	  			<?php if(is_array($searchfids)): foreach($searchfids as $key=>$v): if($searchfid == $v['pid']): ?><option value="<?php echo ($v['pid']); ?>" selected="selected"><?php echo ($v['ptitle']); ?></option>
	  				<?php else: ?>
	  					<option value="<?php echo ($v['pid']); ?>" ><?php echo ($v['ptitle']); ?></option><?php endif; endforeach; endif; ?>
  			</select>
	  	</td>
		<td style="padding-left:1em;">
			<input type="hidden"  id="searchunum" name="searchunum" value="<?php echo ($unum); ?>">
			<input type="hidden"  id="searchuname" name="searchuname" value="<?php echo ($uname); ?>">
	...
	<form method='post' action="__URL__/outputproject" >	
		...
		<?php if(is_array($project)): foreach($project as $key=>$u): ?><tr>
			...
			<td>
				<a href="<?php echo U('Admin/SuperManage/projectread',array('pid'=>$u['pid'],'unum'=>$unum,'uname'=>$uname));?>">读取</a>
