...
<body>
	<div class='container'>
	<h2 class='head'>竞赛信息读取修改</h2>
	<hr/>
	 <form id="form2" method='post' action="__URL__/raceupdate" class="form-horizontal" enctype="multipart/form-data">
	 			<div class="form-group">
				 	<div class="col-sm-10 col-sm-offset-1">
				      <input type='text' class="form-control " id="name" name="name"  value="<?php echo ($race['rname']); ?>">
				    </div>
			    </div>
				<div class="form-group">
					<label for="username" class="col-sm-2 control-label" >开始日期:</label>
				    <div class="col-sm-3">
				      <input type="text" class="form-control " id="datestart" name="datestart"  onClick="WdatePicker()" value="<?php echo ($race['rdatestart']); ?>">
				    </div>
				    <label for="username" class="col-sm-2  control-label" >截止日期:</label>
				    <div class="col-sm-3">
				      <input type="text" class="form-control " id="dateend" name="dateend"  onClick="WdatePicker()" value="<?php echo ($race['rdateend']); ?>">
				    </div>
				</div>
				<hr/>
	 			<div class="form-group">
	 			<label for="usernum" class="col-sm-2 control-label">发布状态:</label>
				    <div class="col-sm-3">
				       <?php if($race['rstatus'] == 1): ?><input type="radio"   name="status" value='1' checked="checked" style="width: 50px"/>&nbsp;&nbsp;是&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<div class="form-group">
					<div class="col-sm-10 col-sm-offset-1">
				      <textarea  id="content" name="content" ><?php echo ($race['rcontent']); ?></textarea>
				    </div>
				</div>
				<div class="form-group">
					<label for="username" class="col-sm-2 control-label" >主办/承办单位:</label>
				    <div class="col-sm-8">
				      <input type="text" class="form-control " id="sponsor" name="sponsor" value="<?php echo ($race['rsponsor']); ?>">
				    </div>
				</div>
				<div class="form-group">
				    ...
				    <div class="col-sm-4">
			   		 	<select id="level" name="level" class="form-control">
			   		 		<?php if($race['rlevel'] == '院级'): ?><option value='院级'  selected="selected">院级</option>
			   		 		<?php else: ?>
			   		 			<option value='院级'>院级</option><?php endif; ?>
			   		 		<?php if($race['rlevel'] == '校级'): ?><option value='校级'  selected="selected">校级</option>
			   		 		<?php else: ?>
			   		 			<option value='校级'>校级</option><?php endif; ?>	
			   		 		<?php if($race['rlevel'] == '市级'): ?><option value='市级'  selected="selected">市级</option>
			   		 		<?php else: ?>
			   		 			<option value='市级'>市级</option><?php endif; ?>	
			   		 		<?php if($race['rlevel'] == '国家级及国家级以上'): ?><option value='国家级及国家级以上'  selected="selected">国家级及国家级以上</option>
			   		 		<?php else: ?>
			   		 			<option value='国家级及国家级以上'>国家级及国家级以上</option><?php endif; ?>								
						</select>
				   </div>
				</div>
					...
					<label for="username" class="col-sm-1  control-label" >附件:</label>
				 	<div class="col-sm-8">
				      请选择需要上传的文件(大小不超过10M，文件类型为.doc,.pdf,.zip)：
					     ...
					     当前文件为：<?php echo ($race["raccessory"]); ?><br/>
					    <?php if($race['raccessory'] == ''): else: ?>
					    	 <a href="<?php echo U('Admin/SuperManage/downracefile');?>?filename=.<?php echo ($race["raccessory"]); ?>">文件下载</a><?php endif; ?>   			    	
				    </div>	
				</div>
		  		<div class="modal-footer">
		   			<input type='hidden' name='racefile' value='<?php echo ($race["raccessory"]); ?>'>
	 			    <input type='hidden',id='rid' name='rid' value='<?php echo ($race["rid"]); ?>'>
