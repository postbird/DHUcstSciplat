	 	...
	 	<form id="form1" method='post'  class="form-horizontal" enctype="multipart/form-data">
					...
					<div class="form-group">
						<?php if($race['raccessory'] == ''): else: ?>
					    	 ...
					    	 <label  class="col-sm-2 control-label normalfont" ><a href="<?php echo U('Admin/Stu/downracefile');?>?filename=.<?php echo ($race["raccessory"]); ?>">附件下载</a></label><?php endif; ?>   
					</div>
					<hr/>
					<div class="form-group">
						<div style="margin:1em auto;width:10%;">
							<?php if($race['subtime'] == 1): ?><label " class="control-label" >已截止</label>
							<?php else: ?>
			  		...
			  		<div class="modal-footer">
		   			 	 <a class="btn btn-primary" href="<?php echo U('Admin/Stu/race',array('unum'=>$unum,'uname'=>$uname));?>" target="opt"><span>返回</span></a>
					</div>
		</form>
	</div>
	<!-- Modal 添加信息开始-->
		  ...
		  <div class="modal-dialog">
		    <div class="modal-content">
		      <div class="modal-header">
		        ...
		        <h4 class="modal-title head" id="myModalLabel"><?php echo ($race['rname']); ?></h4>
		        <h3 class='head'>竞赛申报</h3>
		      </div>
		      <div class="modal-body">
		        <form id="form2" method='post' action="__URL__/raceinsert" class="form-horizontal" enctype="multipart/form-data">
			
				  	<div class="form-group">
				  		<div class="col-sm-10">
					 		<p style="text-align:center;color:red;">*填写说明*</p>
				 			<p>1、竞赛申报的填写由每一小组的队长填写即可，其他成员不用填写；</p>
				 			<p>2、队长添加小组成员时，成员的学号和姓名一定要一致，否则会不通过；</p>
				 			<p>3、上传附件的材料包括证书图片等；</p>
					 	</div>	
				    </div>
					<hr/>
					<div class="form-group">
					...
					<hr/>
					<div class="form-group">
						<label for="username" class="col-sm-2  control-label" >获奖:</label>
						<div class="col-sm-4">
				   		 	<select id="bonus" name="bonus" class="form-control">
								<option value='0'>=请选择获奖级别=</option>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
					     ...
					     <input type='file' name="imageFile" onchange="previewFile()" id='imageread'/>
			      		<img src="" height="200" width="300" alt="图片 预览..."/>	
					    </div>
					   
				  </div>
			  
				   <div class="modal-footer">
				   	<input type='hidden',id='race_name' name='race_name' value='<?php echo ($race["rname"]); ?>'>
				   	<input type='hidden',id='race_level' name='race_level' value='<?php echo ($race["rlevel"]); ?>'>
